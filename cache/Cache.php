...
require 'storage/MMap.php';
require 'storage/ShMop.php';
require 'Hash.php';
...
class Cache
{
    ...
    private function getStorage(...$args) : StorageInterface
    {
    ...
    {
        $setKeyLen = strlen($key);
        if ($setKeyLen > 48) {
            $this->throw('Max key length is 48 bytes');
            return false;
        }
        $i = $hash = $hash1
            = $hash2 = $lru = $lruHash
     ...
     */
    private function getCacheMem(int $hash) : ?StorageInterface
    {
    ...
    {
        return $this->keyMem->write(str_repeat($this->initChr, $this->keySlotsMem), 0) === false ? false : true;
    }
     ...
     * @return null|StorageInterface
     */
    private function getQueueMem(string $queue) : ?StorageInterface
    {
     ...
     * @return bool|null
     */
    public function enQueue(string $queue, string $data) : ?bool
    {
        ...
        if ($rear < $front) {
            ...
            if ($this->maxQueueMem - $rear >= 4
                && $queueMem->write(str_repeat($this->initChr, 4), $rear)) {
                $this->queueUnlock($queue);
                return false;
            }
            $rear = 12;
